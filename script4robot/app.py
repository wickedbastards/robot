...
camera.set(cv2.CAP_PROP_FRAME_WIDTH, 400)   # Определенные разрешения с некоторыми камерами могут не работать, поэтому для
camera.set(cv2.CAP_PROP_FRAME_HEIGHT, 300)  # уменьшения разрешения можно также использовать resize в методе getFramesGenerator
...
def getFramesGenerator():
    ...
    while True:
        ...
        if success:
            frame = cv2.resize(frame, (400, 300), interpolation=cv2.INTER_AREA)  # уменьшаем разрешение кадров (если
            ...
            if iSee:    # если был найден объект
                controlY = 0.8  # начинаем ехать вперед с 50% мощностью 
            else:
...
if __name__ == '__main__':
    ...
    # параметры робота
    speedScale = 0.8  # определяет скорость в процентах (0.60 = 60%) от максимальной абсолютной
