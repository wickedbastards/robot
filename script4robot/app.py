...
camera.set(cv2.CAP_PROP_FRAME_WIDTH, 800)   # Определенные разрешения с некоторыми камерами могут не работать, поэтому для
camera.set(cv2.CAP_PROP_FRAME_HEIGHT, 600)  # уменьшения разрешения можно также использовать resize в методе getFramesGenerator
...
def getFramesGenerator():
    ...
    while True:
        ...
        if success:
            frame = cv2.resize(frame, (800, 600), interpolation=cv2.INTER_AREA)  # уменьшаем разрешение кадров (если
            ...
            if iSee:    # если был найден объект
                controlY = 1  # начинаем ехать вперед с 50% мощностью 
            else:
...
if __name__ == '__main__':
    ...
    # параметры робота
    speedScale = 1  # определяет скорость в процентах (0.60 = 60%) от максимальной абсолютной
